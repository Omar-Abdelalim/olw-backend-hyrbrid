...
async def create_paylink(request: Request, response: Response, payload: dict = Body(...), db: Session = Depends(get_db)):
    ...
    q = QRTer(terminalID = "ecom/"+paylinkID,displayName = r["merchantName"],merchantName=r["merchantName"],merchantAccount=r["merchantAccount"],currency=payload['currency'],qrStatus="active",amount=payload["amount"],dateTime = datetime.now())
    db.add(q)
    db.add(p)
    db.commit()
